...
IF DO_RANGES NE !NULL AND DO_RANGES GT 0 THEN BEGIN & $
  xmin=475 & xmax=525 & ymin=475 & ymax=525 & rangestr=STRCOMPRESS("--"+string(xmin)+"-"+string(xmax)+"--"+string(ymin)+"-"+string(ymax),/remove_all) & $
...
for i = 0,n_elements(segs)-1 do BEGIN & $
  ...
  IF do_powspec then begin & $
    outfname=plotdir+"bispectrum_plot--n_segs" + STRCOMPRESS(cur_segnum,/remove_all) + rangestr + "_pow_spec--FAB--5MHzrate--halfamp_inputwaves--doubleamp_output.ps" & $
  ...
  ENDIF ELSE BEGIN & $
    outfname=plotdir+"bispectrum_plot--n_segs" + STRCOMPRESS(cur_segnum,/remove_all) + rangestr + "--FAB--5MHzrate--halfamp_inputwaves--doubleamp_output.ps" & $
